...
This repository is the **single source of truth** for Ambient foundation assets. **Downstream application repositories must not** maintain a second copy of contract YAML, catalog YAML, generated catalog artifacts, or shared pipeline governance modules.
...
- **`lib/ambient_cli/`** — `validate-contracts`, `ambient-catalog-generate`.
...
1. **One canonical version** — consumers pin a git tag `vX.Y.Z`; all contract/catalog/pipeline changes merge here first, then tag, then bump consumers ([CONTRIBUTING.md](CONTRIBUTING.md#consumer-follow-up-after-a-release)).
...
- `validate-contracts`, `ambient-catalog-generate --check`, `pytest`.
