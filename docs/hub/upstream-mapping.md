...
**Site hub-receiver fetch list:** extending the ambientsystems.ai receiver to fetch new hub files must go through ambient-systems hub-sync of spoke Actions config (template `.github/hub/templates/site-hub-receiver.yml`), or an explicit Attended operator one-time exception — never a direct Ambient-Team agent PR. Current mirrored cascade proof is `spokes-site-readme` plus `hub-hypotheses`. Attended 2026-08-12 exception: also fetch `spokes/site/messaging/boundary.md` (master-hub boundary bridge) into `content/messaging/boundary.md`. Apply via workflow_dispatch **Apply site hub-receiver fetch list** (`.github/workflows/apply-site-hub-receiver-fetch.yml`) which opens a site `hub-sync/*` PR using `HUB_DISPATCH_AMBIENT_TEAM`. That PAT needs **Workflows** write on `Ambient-Team/ambientsystems.ai` (Contents/PR alone is rejected for `.github/workflows/hub-receiver.yml` pushes). Run 31599551149 failed on that scope gap; re-run after the PAT is updated.
