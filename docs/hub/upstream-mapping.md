...
- **FETCH_MASTER** — Contents: Read on EngineerID/master-hub. Used by **Master hub sync receiver**.

Upstream cascade: event `master_hub_sync` → `master-hub-receiver.yml` → `interface/master-hub/**` → sync rule `master-hub-oversight` → Ambient-Team. See master-hub `docs/remote-control.md`.
