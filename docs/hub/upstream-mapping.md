...
**Site hub-receiver fetch list:** extending the ambientsystems.ai receiver to fetch new hub files (for example `cascade-note`) must go through a future gated workflow sync or an explicit operator one-time exception. Current mirrored cascade proof is `spokes-site-readme` plus `hub-hypotheses`.
