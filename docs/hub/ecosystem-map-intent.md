...
- Role: Doctoral theory root — one-way; only defended research production may enter Ambient core via derivation map
...
- Read-only. Only defended doctoral research production may become Ambient core. Not-yet-defended material is not used in core; if required at all, peripheral only at minimum necessary scope. Never reverse-sync Ambient proprietary into PhD.
